...
现场故障与已验证处理方法统一记录在 [troubleshooting.md](troubleshooting.md)。
...
| Arduino ESP32 Core | 3.3.11 | WiFi / HTTP / mbedTLS / NTP |

## 核心功能

1. **短信接收** — 4G 模组 PDU 模式自动上报，解析后提取发送方、设备接收时间、接收 Profile 与内容
...
6. **系统日志** — Web 端实时查看设备串口日志，自动刷新，最多保留 80 行且单行最多 320 字节
...
| 企业微信机器人 | `PUSH_TYPE_WECOM` (11) | 群机器人 Webhook，支持可选关键词 |
...
├── auth.h / .cpp         # Cookie 会话、CSRF 防护、登录限流
├── api_handlers.h / .cpp # 状态、短信、eSIM、运营商与配置 API
...
3. 模组断电重启（EN 引脚控制）
4. 加载 NVS 配置并挂载 LittleFS 短信存储
5. 连接 Wi-Fi，启动 HTTP 管理台并同步 NTP 时间
6. 若配置有效，发送启动通知邮件
7. 模组初始化：AT 握手 → 禁用数据连接 → 读取并脱敏缓存 ICCID 尾号 → 配置短信 URC 上报 → 设置 PDU 模式 → 等待网络注册
8. 启动 eSIM 与运营商状态管理任务
...
- **日志环形缓冲区**：80 行容量、单行 320 字节上限，循环覆盖，通过 `/log` 端点以 JSON 返回，Web 端每 2 秒自动刷新
