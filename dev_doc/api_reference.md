...
3. `smsStoreAdd()` — 持久化发送方、接收 Profile、设备接收时间与正文
4. `sendSMSToServer()` — 推送所有启用的通道
5. `sendEmailNotification()` — 邮件通知
...
## 模块: api_handlers.cpp — JSON API

### `void handleApiStatus()`
**用途**: HTTP 端点 `GET /api/status`，返回仪表盘使用的实时设备状态。蜂窝信号来自 SIM 管理器异步缓存，不在 HTTP 请求中同步执行 AT 指令。
...
`rsrp` 与 `signal.dbm` 的单位为 dBm，`rsrq` 的单位为 dB；`raw` 是
