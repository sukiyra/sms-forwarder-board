...
路由分发:
    ...
    GET  /api/status → handleApiStatus()  读取缓存的设备/SIM/蜂窝信号状态
...
`simManagerLoop()` 在 SIM 与短信服务就绪后，每 10 秒异步发送一次
`AT+CESQ`。解析后的 LTE RSRP/RSRQ 保存在内存中，`GET /api/status`
只读取缓存，不会因网页每 3 秒轮询而占用短信 UART。缓存超过 45 秒，或 SIM
拔出/状态失效后，接口返回未知信号。
