...
- 响应式 Web 管理台，适配手机、普通桌面和超宽屏；仪表盘并列显示蜂窝与 Wi-Fi 五级信号图，跨页面保留 OTA、选网、切卡和短信发送进度。
- 设备健康面板实时显示堆内存占用、启动以来最低余量、最大连续内存块、碎片率、LittleFS 占用、运行时间和上次复位原因；低内存、严重碎片或存储异常会标红。
...
短信记录固定为 50 条循环存储，系统日志固定为 80 条且每条最多 320 字节，运营商扫描、长短信和任务队列也使用固定容量。管理台的健康面板会保留 ESP32 自启动以来的最低空闲堆指标，用于发现缓慢泄漏；量产和长期运行仍应使用实际电源、SIM 与网络做持续老化测试。

从旧分区升级时，必须先用当前量产包通过 USB 完整烧录一次分区表。只写入应用 BIN 或从已装在旧分区上的 v1.6.3 在线升级，都会保留旧分区表，无法创建 LittleFS；管理台会明确显示“分区需迁移”。完成这次迁移后，后续版本可直接在线升级。需要保留现有配置时，在量产工具中关闭“全片擦除”；NVS 地址保持不变。
